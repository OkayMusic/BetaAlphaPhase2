# $Id: addcmds.tcl 1251 2014-03-10 22:17:29Z toby $
...
proc MakeAddPhaseBox {} {
    ...
    grid [frame $np.ct -bd 2 -relief groove] -column 3 -row 2
    grid [frame $np.f -bd 4 -relief groove] -column 4 -row 2 -columnspan 8
    grid [label $np.ct.l -text "Cell\nType"] -column 0 -row 2 
    set menu [tk_optionMenu $np.ct.s expgui(addPhaseCellType) ""]
    $menu delete 0
    foreach val {Any Cubic Tetragonal Hexagonal Rhombohedral Orthorhombic Monoclinic} \
	lbl {Any Cubic Tetrag Hexag Rhomb. Ortho Mono} {
	    $menu add radiobutton -value $lbl -label $val \
		-command "set expgui(addPhaseCellType) $lbl"
	}
    grid $np.ct.s
    foreach v [ trace vinfo expgui(addPhaseCellType)] {
	eval trace vdelete expgui(addPhaseCellType) $v
    }
    #set expgui(addPhaseCellType) Any
    $menu invoke 0
    trace variable expgui(addPhaseCellType) w "SetAddCellSymConstr $np.f"
...
proc SetAddCellSymConstr {box args} {
    switch $::expgui(addPhaseCellType) {
	Any {
	    foreach i {a b c} {
		$box.e1$i configure -textvariable "" -state normal
	    }
	    foreach i {a b g} {
		$box.e2$i configure -textvariable "" -state normal
	    }
	}
	Cubic {
	    set ::expgui(AddCella) [$box.e1a get]
	    foreach i {a b c} {
		$box.e1$i configure -textvariable expgui(AddCella) -state normal
	    }
	    foreach i {a b g} {
		$box.e2$i configure -textvariable "" -state normal
		$box.e2$i delete 0 end
		$box.e2$i insert 0 90.
		$box.e2$i configure -textvariable "" -state disabled
	    }
	}
	Tetrag {
	    set ::expgui(AddCella) [$box.e1a get]
	    ...
	    $box.e1c configure -textvariable "" -state normal
	    foreach i {a b g} {
		$box.e2$i configure -textvariable "" -state normal
		$box.e2$i delete 0 end
		$box.e2$i insert 0 90.
	...
	Rhomb. {
	    ...
	    set ::expgui(AddCellalpha) [$box.e2a get]
	    foreach i {a b g} {
		$box.e2$i configure -textvariable expgui(AddCellalpha) -state normal
	    }
	}
	Ortho {
	    foreach i {a b c} {
	...
	Mono {
	    ...
	    $box.e2b configure -textvariable "" -state normal
	}
    }
}
...
proc addphase {np} {
    ...
    # check if standard and add spaces, as needed
    if {[set nspg [CheckSpg $spg]] != ""} {
        set spg $nspg
        $np.t2 delete 0 end
        $np.t2 insert end $nspg
    }
    foreach i {a b c} {
    ...
    if {$expgui(addPhaseCellType) == "Rhomb."} {
	# check for proper use of space group in Rhomb. setting
	if {[string toupper [string range [string trim $spg] 0 0]] != "R"} {
	    append err "Rhombohedral cell specified, but space group does not start with R\n"
	}
	if {[string toupper [string range [string trim $spg] end end]] != "R"} {
	    append spg " R"
	    if {$err == ""} {
    ...
    # ok do it!
    set errmsg [runAddPhase $title $spg \
		    $cell(a) $cell(b) $cell(c) \
		    $cell(alpha) $cell(beta) $cell(gamma)]
    RecordMacroEntry "runAddPhase [list $title] [list $spg] \
		    $cell(a) $cell(b) $cell(c) \
		    $cell(alpha) $cell(beta) $cell(gamma)" 0
    destroy $np
    ...
    set phase [lindex $expmap(phaselist) end]
    SelectOnePhase $phase
    SpaceGroupWarnings $warn $phase $spg
}

proc runAddPhase {title spg a b c alpha beta gamma} {
    global expgui expmap tcl_platform
    set fp [open exptool.in w]
    puts $fp "P"
    puts $fp $title
    puts $fp $spg
    puts $fp "$a $b $c $alpha $beta $gamma"
    puts $fp "/"
    close $fp
    ...
    close $fp
    catch {file delete exptool.in exptool.out}

    if {[llength $oldphaselist] == [llength $expmap(phaselist)]} {
	set err 1
	if {$errmsg == ""} {set errmsg "No phase added"}
    }
    if {$err} {
	return $errmsg
    } else {
...
proc MakeAddHistBox {} {
    global expmap newhist expgui

    # --> should check here if room for another histogram, but only texture
    # folks will ever need that


    # set if the CIF import loads OK
    set OK 1
    catch {
	if {[info procs ReadCIF4GSAS] == ""} {
	    set OK 0
	    source [file join $::expgui(scriptdir) cif2fxye.tcl]
	}
    } errmsg
    ...
    if {$OK} {
	grid [button $np.b1a -text "Import CIF" \
		  -command "ReadCIF4GSAS $np" \
		 ] -column 5 -row 2
    }
    ...
    # fix grab...
    afterputontop
    # if no histogram is selected, select the last
    if {$expgui(curhist) == "" && $expmap(powderlist) != ""} {
	$expgui(histFrame).hs.lbox select set end
	set expgui(curhist) [$expgui(histFrame).hs.lbox curselection]
	DisplayHistogram
    }
}

# convert a file to Win-95 direct access -- this is not used anymore
...
proc getrawfile {np} {
	...
	tk_getOpenFile -parent $np -initialfile $newhist(rawfile) -filetypes {
	    ...
	    {"Data files" .RAW}  {"Data files" .FXYE} {"All files" *}
	}
	]
    } else {
	set inp [
	tk_getOpenFile -parent $np -initialfile $newhist(rawfile) -filetypes {
	    ...
	    {"Data files" .gsa*} {"Data files" .raw} {"Data files" .fxye}   
	    {"All files" *}
	} 
	]
    }
...
proc validaterawfile {np inp} {
	...
	grid [radiobutton $np.bank.$i -text $i -command SetDefaultBank \
		-variable newhist(banknum) -value $i] \
		-column [incr col] -row $row -sticky w
    }
    # if only 1 choice, set it
...
proc SetDefaultBank {} {
    # set the default instrument parameter set to follow the bank # when the 
    # numbers of banks in each place agree. Do this when the data bank # is set
    global newhist
...
proc SetTmax {} {
    # set the default data range when the data or instrument parameter set is changed
    global newhist
	...
	# value from the instrument parameter file.
	catch {
	    set s $newhist(setnum)
	    foreach {x tmin tmax x} $newhist(inst${s}ITYP) {}
	    if {$tmin > 0} {set newhist(2tLimit) $tmin}
	}
    } else {
...
proc validateinstfile {np inp} {
	...
	grid [radiobutton $np.set.$i -text $i \
		-command "PostInstBankopts $np; ValidateDummyHist $np" \
		-variable newhist(setnum) -value $i] \
		-column [incr col] -row $row -sticky w
	if {$newhist(instbanks) == 1} {set newhist(setnum) $i}
    }
    if {$newhist(dummy)} {PostInstBankopts $np; ValidateDummyHist $np}
    LabelInstParm
    SetMultipleAdd $np
}
...
proc addhist {np} {
    ...
    # ok do it!
    set errmsg [runAddHist $newhist(rawfile) $newhist(instfile) $newhist(banknum) $newhist(setnum) $mode $value]
    # save call to Macro file
    RecordMacroEntry "runAddHist [list $newhist(rawfile)] [list $newhist(instfile)] $newhist(banknum) $newhist(setnum) $mode $value" 0
	...
	if {$errmsg != ""} {append msg "\nIt appears an error occurred!"}
	ShowBigMessage $np $msg $errmsg OK "" $errcode
    }
...
proc runAddHist {rawfile instfile banknum setnum mode value} {
    global expgui expmap tcl_platform
    set fp [open exptool.in w]
    puts $fp "H"
    if {$tcl_platform(platform) == "windows"} {
        if {[string length $rawfile] > 50} {
            puts $fp [file attributes $rawfile -shortname]
        } else {
            puts $fp $rawfile
        }
        if {[string length $instfile] > 50} {            
            puts $fp [file attributes $instfile -shortname]
        } else {
            puts $fp $instfile
        }
    } else {
    ...
    loadexp $expgui(expfile)
    if {[llength $oldpowderlist] == [llength $expmap(powderlist)]} {
	append errmsg "\nNo histogram added"
	set err 1
    }
    set fp [open exptool.out r]
...
#--- Respond to the setting of the bank # for the Instrument parameter file.
proc PostInstBankopts {np} {
    global newhist
    if {$newhist(dummy)} {
...
proc AddDummyHist {np} {
    ...
    set errmsg [runAddDummyHist $newhist(instfile) $newhist(setnum) \
		    $newhist(insttype) \
		    $newhist(tmin) $newhist(tmax) $newhist(tstep)]
    RecordMacroEntry "runAddDummyHist [list $newhist(instfile)] $newhist(setnum) $newhist(insttype) $newhist(tmin) $newhist(tmax) $newhist(tstep)" 0

    if {[regexp {\(P,H,A\)} $expgui(exptoolout)]} {
	...
	destroy $np
    } elseif {$expgui(showexptool) || $errmsg != ""} {
  	if {$errmsg != ""} {
	    set err 1
	    append errmsg "\n" $expgui(exptoolout) 
	} else {
...
proc runAddDummyHist {instfile setnum insttype tmin tmax tstep} {
    global expgui expmap tcl_platform
    set fp [open exptool.in w]
    puts $fp "D"
    puts $fp $instfile
    ...
    if {[llength $oldpowderlist] == [llength $expmap(powderlist)]} {
	set err 1
	if {$errmsg == ""} {set errmsg "No histogram added"}
    }
    if {$err} {
	return $errmsg
    } else {
...
proc addMultiplehist {np} {
    ...
    grid [label $top.top \
	      -text "Select banks to add; set the\nuseful range of data, limiting\nthe # of reflections." -anchor w -justify left \
	      -bg beige] -column 0 -columnspan 3 -row 0 -sticky ew
    ...
    grid [radiobutton $top.vartyp.cb3 -text "d-min\n(A)" -variable newhist(LimitMode) \
    ...
    grid [radiobutton $top.vartyp.cb5 -text "Q-max\n(A-1)" -variable newhist(LimitMode) \
	...
	if {$newhist(insttype) == "TOF"} {
		...
		foreach {x tmin tmax x} $newhist(inst${i}ITYP) {}
		if {$tmin > 0} {set newhist(tlimit$i) $tmin}
	    }
	} else {
	...
	if {$newhist(usebank$i)} {
	    ...
	    set errmsg [runAddHist $newhist(rawfile) $newhist(instfile) $i $k $mode $value]
	    # save call to Macro file
	    RecordMacroEntry "runAddHist [list $newhist(rawfile)] [list $newhist(instfile)] $i $k $mode $value" 0
	    if {$errmsg != ""} {
		append outlog "\n\n\nNOTE ERROR:\n" $errmsg $expgui(exptoolout)
		set err 1
	    } else {
		append outlog $expgui(exptoolout)
	    }
	}
    }
    # load the revised exp file
...
set expgui(atmtypelist) {}
proc GetAtmTypes {} {
    set ::expgui(atmtypelist) {}
    set ::expgui(magtypelist) {}
    catch {
	set fp [open [file join $::expgui(gsasdir) data atmdata.dat] r]
	while {[gets $fp line] >= 0} {
	    set token [string toupper [string trim [string range $line 2 8]]]
	    if {$token == ""} continue
	    # nuclear/magnetic
	    if {[lindex $token 1] == "N" || [lindex $token 1] == "M"} {
		set magarr([lindex $token 0]) 1
		continue
	    }
	    if {[string is integer $token]} continue
	    if {[string first "_" $token] != -1} {
		set sp [split $token "_"]
		if {[lindex $sp 1] == ""} continue
		if {! [string is integer [lindex $sp 1]]} continue
	    }
	    set tmparr([string toupper $token]) 1
	}
	set ::expgui(atmtypelist) [array names tmparr]
	set ::expgui(magtypelist) [array names magarr]
	close $fp
    } errmsg
    if {$::expgui(debug) && $errmsg != ""} {
	puts "GetAtmTypes error: $errmsg"
    }
}
...
proc ValidateAtomsBox {top np} {
	...
	CE+3 CE+4 PR PR+3 PR+4 PR_141 ND ND+3 PM PM+3 SM SM+3 SM_152
	...
	AT AT_210 RN RN_222 FR FR_223 RA RA+2 RA_226 AC AC+3 AC_227 TH
	...
	if {[lsearch $validatmtypes [string toupper $type]] == -1} {
	    if {[llength $expgui(atmtypelist)] == 0} GetAtmTypes
	    if {[lsearch $expgui(atmtypelist) [string toupper $type]] == -1} {
		if {[lsearch $expgui(magtypelist) [string toupper $type]] == -1} {
		    append err "  line $row: Atom type $type is not defined in GSAS"
                    set sym2 [string range [string toupper $type] 0 1]
                    set sym1 [string range [string toupper $type] 0 0]
                    if {[lsearch $expgui(atmtypelist) $sym2] != -1} {
                        $np.e${row}t delete 0 end
                        $np.e${row}t insert 0 $sym2
                        append err "; changed to $sym2.\n\n"
                    } elseif {[lsearch $expgui(atmtypelist) $sym2] != -1} {
                        $np.e${row}t delete 0 end
                        $np.e${row}t insert 0 $sym1
                        append err "; changed to $sym1.\n\n"
                    } else {
                        append err ".\n\n"
                    }
		} else {
		    append err "  line $row: Atom type $type is not defined for x-rays in GSAS, but is defined for magnetic/nuclear scattering. If you want to use this symbol choose a different type here and change it later in EXPEDT.\n\n"
		}
	    }
	}
...
proc runAddAtoms {phase atomlist} {
    global expgui env expmap tcl_platform
    # needed in UNIX
	...
	set err 1
	if {$errmsg == ""} {set errmsg "No atom(s) added"}
    } elseif {! $::expgui(debug)} {
        catch {file delete exptool.in exptool.out}
    }
    return $errmsg
...
proc addatom {phase top} {
    ...
    # ok add the atoms!
    set errmsg [runAddAtoms $phase $atomlist]
    RecordMacroEntry "runAddAtoms $phase [list $atomlist]" 0
...
proc MakeXformAtomsBox {phase} {
    ...
    # allow fixing of atom coordinates
    if {[lindex $expmap(phasetype) [expr {$p - 1}]] != 4} {
	grid [TitleFrame $w.10 -bd 6 -relief groove \
                  -text "Fix Coordinates for Atom$suffix"] \
            -row 9 -column 0 -columnspan 10 -sticky news
	set fix [$w.10 getframe]
        # set button labels
        set ::fixbtn_lbl(X) [Fix_State $phase $numberList X]
        set ::fixbtn_lbl(Y) [Fix_State $phase $numberList Y]
        set ::fixbtn_lbl(Z) [Fix_State $phase $numberList Z]

        label $fix.xlab -text "  x  " -width 8
        label $fix.ylab -text "  y  " -width 8
        label $fix.zlab -text "  z  " -width 8
        grid $fix.xlab -row 2 -column 0
        grid $fix.ylab -row 2 -column 1
        grid $fix.zlab -row 2 -column 2


        button $fix.x -textvariable fixbtn_lbl(X) -width 8 \
            -command "Fix_Atoms $phase [list $numberList] X"
        button $fix.y -textvariable fixbtn_lbl(Y) -width 8 \
            -command "Fix_Atoms $phase [list $numberList] Y"
        button $fix.z -textvariable fixbtn_lbl(Z) -width 8 \
            -command "Fix_Atoms $phase [list $numberList] Z"
        grid $fix.x -row 3 -column 0
        grid $fix.y -row 3 -column 1
        grid $fix.z -row 3 -column 2
    }
...
# test the fixed status of variable for several atoms
proc Fix_State {phase numberList coord} {
    set status_fixed 0
    set status_unfixed 0
    #puts "$coord before: $status_fixed $status_unfixed"
    foreach i $numberList {
        set temp [atom_constraint_get $phase $i $coord]
        if {$temp == 0} {
            set status_unfixed 1
        } else {
            set status_fixed 1
        }
        if {$status_fixed == 1 && $status_unfixed == 1} {
            return "fix\nsome"
        }
    }
    #puts "$coord after $status_fixed $status_unfixed"
    if {$status_fixed == 0} {return "fix"}
    return "release"
}

# fix or release the selected atoms
proc Fix_Atoms {phase numberList coord} {
    if {$::fixbtn_lbl($coord) == "release"} {
        set ::fixbtn_lbl($coord) "fix"
        set mode 0
    } else {
        set ::fixbtn_lbl($coord) "release"
        set mode 1
    }
    atom_constraint_set $phase $numberList $coord $mode
    incr ::expgui(changed)
    RecordMacroEntry "atom_constraint_set $phase [list $numberList] $coord $mode" 0
...
proc XformAtomsCoord {phase numberList w1} {
	...
	foreach v {x y z} {
	    ...
	    RecordMacroEntry "$cmd $phase $atom $v set [set new$v]" 0
	}
	incr expgui(changed)
    }
    RecordMacroEntry "incr expgui(changed)" 0
...
proc XformAtomsOcc {phase numberList w2} {
	...
	RecordMacroEntry "$cmd $phase $atom frac set $val" 0
	incr expgui(changed)
    }
    RecordMacroEntry "incr expgui(changed)" 0
    UpdateAtomLine $numberList $phase
...
proc XformAtomsU {phase numberList w2} {
    global expgui
    set istart $expgui(changed)
    if {$w2 == "iso"} {
	foreach atom $numberList {
	    if {[atominfo $phase $atom temptype] != "I"} {
		atominfo $phase $atom temptype set I
		RecordMacroEntry "atominfo $phase $atom temptype set I" 0
		incr expgui(changed)
	    }
	}
    } elseif {$w2 == "aniso"} {
	foreach atom $numberList {
		...
		RecordMacroEntry "atominfo $phase $atom temptype set A" 0
		incr expgui(changed)
	    }
	}
    } else {
	...
	foreach atom $numberList {
		...
		RecordMacroEntry "mmatominfo $phase $atom Uiso set $val" 0
	    } elseif {[atominfo $phase $atom temptype] == "I"} {
		atominfo $phase $atom Uiso set $val
		RecordMacroEntry "atominfo $phase $atom Uiso set $val" 0
	    } else {
		foreach var {U11 U22 U33} {
		    atominfo $phase $atom $var set $val
		    RecordMacroEntry "atominfo $phase $atom $var set $val" 0
		}
		foreach var {U12 U13 U23} {
		    atominfo $phase $atom $var set 0.0
		    RecordMacroEntry "atominfo $phase $atom $var set 0.0" 0
		}
	    }
	    incr expgui(changed)
	}
    }
    if {$istart != $expgui(changed)} {RecordMacroEntry "incr expgui(changed)" 0}
    UpdateAtomLine $numberList $phase
}
...
proc EraseAtoms {phase numberList w2} {
	...
	foreach atom $numberList {
	    ...
	    RecordMacroEntry "EraseAtom $atom $phase" 0
	    incr expgui(changed)
	}
	RecordMacroEntry "incr expgui(changed)" 0
	mapexp
	RecordMacroEntry "mapexp" 0
	DisplayAllAtoms $phase
	destroy $w2
    }
}
...
proc ImportPhase {format np} {
    global expgui
    if {[llength $expgui(extensions_$format)] == 1} {
	lappend typelist [list $format $expgui(extensions_$format)]
    } else {
	foreach item $expgui(extensions_$format) {
	    lappend typelist [list "All $format" $item]
	    lappend typelist [list "$format $item" $item]
	}
    }
    ...
    set spg [lindex $input 0]
    set expgui(coordList) [lindex $input 2]
    set msg [lindex $input 3]

    $np.t2 delete 0 end
    if {$spg == ""} {
        set msg "Warning: a Space Group must be specified."
    } elseif {[set nspg [CheckSpg $spg]] == ""} {
        $np.t2 insert end $spg
        if {$msg != ""} {append msg " "}
        append msg "Warning: this space group ($spg) is not recognized as standard short H-M name. It may be correct, but check it carefully."
    } else {
        $np.t2 insert end $nspg
    }
	...
	grid [label $np.msg -text $msg -fg red -anchor center -bd 4 -relief raised -wraplength 400] \
...
proc MakeReplacePhaseBox {} {
    ...
    grid [radiobutton $top.r1 -text "Keep atoms in phase"\
...
proc replacephase2 {phase top spg cell} {
    ...
    set errmsg [replacephase3 $phase $spg $cell $atomlist]
...
proc replacephase3 {phase spg cell atomlist} {
    global expgui expmap
    # replace spacegroup and cell
    if $::expgui(debug) {puts "phaseinfo $phase spacegroup set $spg"}
    phaseinfo $phase spacegroup set $spg
    RecordMacroEntry "phaseinfo $phase spacegroup set [list $spg]" 0
    foreach val $cell var {a b c alpha beta gamma} {
	phaseinfo $phase $var set $val
	RecordMacroEntry "phaseinfo $phase $var set $val" 0
    }
    incr expgui(changed) 
    # delete all atoms
    foreach i $expmap(atomlist_$phase) {
	EraseAtom $i $phase
	RecordMacroEntry "EraseAtom $i $phase" 0
	incr expgui(changed)
    }
    set expmap(atomlist_$phase) {}
    RecordMacroEntry "incr expgui(changed)" 0
    # write new atoms from table as input to exptool
    set errmsg [runAddAtoms $phase $atomlist]
    RecordMacroEntry "runAddAtoms $phase [list $atomlist]" 0
    SpaceGroupWarnings "" $phase $spg
    return $errmsg
}

proc SpaceGroupWarnings {warn phase spg} {
    # warning on possible origin 1 setting
    set shift [GetOrigin1Shift $phase]
    if {$shift != ""} {
	append warn "Note that this space group ($spg) has both Origin 1 and Origin 2 settings. Origin 2 must be used in GSAS. Please check atom multiplicities (or use the Results/composition command) to verify you have the correct origin setting. Note that the Xform Atoms/Xform Origin 1 to Origin 2 button can be used to correct this.\n"
        ...
        append warn "Note that this space group is not found as a standard short Hermann–Mauguin name. It still may be correct as input or might not be what you want. After inputting atoms, confirm that the computed density is in the expected range and that atoms have the correct site multiplicities."
    }
    if {$warn != ""} {
	MyMessageBox -parent . -title "Space Group Note" \
	    -message $warn -icon warning
    }
}
...
#	puts "$phase $numberList $w1 $shift"
	...
	foreach v {x y z} vs $shift {
	    ...
	    RecordMacroEntry "$cmd $phase $atom $v set [expr {$c + $vs}]" 0
	}
	incr expgui(changed)
    }
...
proc ResetMultiplicities {phase parent} {
    global expgui
    set errmsg [RunResetMultiplicities $phase]
    RecordMacroEntry "RunResetMultiplicities $phase" 0
...
proc RunResetMultiplicities {phase} {
    global expgui tcl_platform
...
set spglist {
    ...
    "P 2" "P 21" "P m" "P a" "P c" "P n" "P 2/m" "P 21/m" 
    "P 2/c" "P 2/a" "P 2/n" "P 21/c" "P 21/a" "P 21/n" 

    "C 2" "C m" "C c" "C n" "C 2/m" "C 2/c" "C 2/n" 

    "P 2 2 2" "P 2 2 21" "P 2 21 2" "P 21 2 2" "P 21 21 2" "P 21 2 21" 
    "P 2 21 21" "P 21 21 21" "P m m 2" "P m 2 m" "P 2 m m" "P m c 21" 
    "P c m 21" "P 21 m a" "P 21 a m" "P b 21 m" "P m 21 b" "P c c 2" 
    "P 2 a a" "P b 2 b" "P m a 2" "P b m 2" "P 2 m b" "P 2 c m" 
    "P c 2 m" "P m 2 a" "P c a 21" "P b c 21" "P 21 a b" "P 21 c a" 
    "P c 21 b" "P b 21 a" "P n c 2" "P c n 2" "P 2 n a" "P 2 a n" 
    "P b 2 n" "P n 2 b" "P m n 21" "P n m 21" "P 21 m n" "P 21 n m" 
    "P n 21 m" "P m 21 n" "P b a 2" "P 2 c b" "P c 2 a" "P n a 21" 
    "P b n 21" "P 21 n b" "P 21 c n" "P c 21 n" "P n 21 a" 
    "P n n 2" "P 2 n n" "P n 2 n" "P m m m" "P n n n" "P c c m" 
    "P m a a" "P b m b" "P b a n" "P n c b" "P c n a" 
    ...
    "P b a m" "P m c b" "P c m a" "P c c n" "P n a a" "P b n b" 
    "P b c m" "P c a m" "P m c a" "P m a b" "P b m a" "P c m b" 
    "P n n m" "P m n n" "P n m n" "P m m n" "P n m m" "P m n m" 
    "P b c n" "P c a n" "P n c a" "P n a b" "P b n a" "P c n b" 
    "P b c a" "P c a b" "P n m a" "P m n b" "P b n m" "P c m n" 
    "P m c n" "P n a m" 

    "C 2 2 21" "C 2 2 2" "C m m 2" "C m c 21" "C c c 2" "C m 2 m" "C 2 m m" 
    "C m 2 a" "C 2 m b" "C 2 c m" "C c 2 m" "C 2 c m" "C c 2 m" 
    "C m c a" "C m m m" "C c c m" "C m m a" "C c c a" "C m c m" 

    "I 2 2 2" "I 21 21 21" "I m m m" "I m m 2" "I m 2 m" "I 2 m m" 
    "I b a 2" "I 2 c b" "I c 2 a" "I m a 2" "I b m 2" "I 2 m b" 
    "I 2 c m" "I c 2 m" "I m 2 a" "I b a m" "I m c b" "I c m a" 
    "I b c a" "I c a b" "I m m a" "I m m b" "I b m m " "I c m m" 
    "I m c m" "I m a m" 
    
    "F 2 2 2" "F m m m"  "F d d d" "F m m 2" "F m 2 m" "F 2 m m" 
    "F d d 2" "F d 2 d" "F 2 d d"

    "P 4" "P 41" "P 42" "P 43" "P -4" "P 4/m" "P 42/m" "P 4/n" "P 42/n" 
    ...
    "I 4" "I 41" "I -4" "I 4/m" "I 41/a" "I 4 2 2" "I 41 2 2" "I 4 m m" 
    ...
    "P 3" "P 31" "P 32" "P -3" "P 3 1 2" "P 3 2 1" "P 31 1 2" 
    ...
    "P 2 3" "P 21 3" "P m 3" "P n 3" "P a 3" "P 4 3 2" "P 42 3 2" 
    "P 43 3 2" "P 41 3 2" "P -4 3 m" "P -4 3 n" "P m 3 m" "P n 3 n" 
    "P m 3 n" "P n 3 m" 

    "I 2 3" "I 21 3" "I m -3" "I a -3"  "I 4 3 2" "I 41 3 2" 
    "I -4 3 m"  "I -4 3 d" "I m -3 m" "I a -3 d"

    "F 2 3" "F m -3" "F d -3" "F 4 3 2" "F 41 3 2" "F -4 3 m" 
    "F -4 3 c" "F m -3 m" "F m -3 c" "F d -3 m" "F d -3 c"
}

# check a space group ignoring spaces to see if it matches a 
# standard one, if so adjust spaces. 
# retain final "R" for rhombodedral settings, but remove the optional "H"
proc CheckSpg {spg} {
    set spg [string trim $spg]
    set lastchar [string toupper [string range $spg end end]]
    if {$lastchar == "R"} {
        # truncate cell setting code
        set spg [string range $spg 0 end-1]
    } elseif {$lastchar == "H"} {
        set spg [string range $spg 0 end-1]
        set lastchar ""
    } else {
    ...
    regsub -all { } [string tolower $spg] {} spg
    foreach s $::spglist {
        if {$spg == [regsub -all { } [string tolower $s] {}]} {
