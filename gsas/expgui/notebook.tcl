...
#
# This version have been modified by Brian Toby to 
#        define Notebook:resize
#        deactivate tabs for notebook pages that have "-state disabled"
#        define a -command configuration option 
...
proc Notebook:create {w args} {
  ...
  set cnt 0
  # initalize commands and state for each page
  foreach p $Notebook($w,pages) {
      set Notebook($w,t$cnt,state) 1
      set Notebook($w,c$cnt,command) {}
      incr cnt
  }
}
...
proc Notebook:config {w args} {
  ...
  foreach {tag value} $args {
    switch -- $tag {
	  ...
	  # reinitalize commands and state for each page
	  set cnt 0
	  foreach p $Notebook($w,pages) {
	      set Notebook($w,t$cnt,state) 1
	      set Notebook($w,c$cnt,command) {}
  ...
  foreach p $Notebook($w,pages) {
    ...
    if [catch {set Notebook($w,c$cnt,title)}] {set Notebook($w,c$cnt,title) $p}
    set id [$w create text 0 0 -text $Notebook($w,c$cnt,title) -anchor nw -tags "p$cnt t$cnt"]
    set bbox [$w bbox $id]
...
proc Notebook:raise.page {w n} {
  ...
  if {$n<0 || $n>=[llength $Notebook($w,pages)]} return
    if {! $Notebook($w,t$n,state)} return
  set top $Notebook($w,top)
  ...
  raise $w.f$n
  if {$Notebook($w,c$n,command) != {}} {
      if [catch {eval uplevel #0 {$Notebook($w,c$n,command)} } errmsg] {
	  catch {puts "Notebook command error: $errmsg"}
      }
  }
}
  ...
  foreach {tag value} $args {
    switch -- $tag {
      -state {
	    ...
	    set Notebook($w,t$i,state) 0
        } else {
          $w itemconfig t$i -fill $Notebook($w,fg,on)
	    set Notebook($w,t$i,state) 1
        }
      }
      -command {
        set Notebook($w,c$i,command) $value
      }
      -title {
        set Notebook($w,c$i,title) $value
      }
      -onexit {
...
#
# Resize the notebook so that it fits the contents.
#
proc Notebook:resize w {
    update idletasks
    set maxw [lindex [$w bbox all] 2]
    set deltah 0
    foreach page [place slaves $w] {
	set h [expr [winfo reqheight $page] - [winfo height $page]]
	set deltah [expr ($deltah > $h) ? $deltah : $h]
	set wid [winfo reqwidth $page]
	set maxw [expr ($maxw > $wid) ? $maxw : $wid]
    }
    $w config -width $maxw -height [expr [winfo height $w] + $deltah]
}
